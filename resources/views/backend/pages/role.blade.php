						...
						<div class="row">
							<div class="col">
								<form action="<?php echo(\Config::get('app.url').'/role_insert')?>" method="post">
										...
										<div class="form-group col-md-12">
											...
											<select class="js-example-basic-multiple form-control" name="permissions[]"
												...
												<option value="{{$all_permission->id}}" @if(!empty($role_update_info))
													@if(!empty($role_update_info['roles_has_permission']))
													@foreach($role_update_info['roles_has_permission'] as
													$roles_has_permission) @if($all_permission->
													id==$roles_has_permission->permission_id)
									...
									<button type="submit" id="role_button" class="btn btn-primary"><i
											class="fas fa-save">
					...
					<table id="role_list" class="table mb-0">
						...
						<tbody>
							...
							<tr>
								<td><?= $i; ?></td>
								<td>{{ $role['role_name'] }}
									<input type="hidden" name="" id="role_name{{$role['role_id']}}"
								...
								<td>
									...
									<a class="btn btn-info"
										href="<?php echo(\Config::get('app.url').'role/'.$role['role_id']);?>"><i
